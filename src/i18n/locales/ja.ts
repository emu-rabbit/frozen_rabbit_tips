export default {
  migration: {
  "title": "お引っ越ししました",
  "description": "旧サイトのデータは自動で移行されません。バックアップをダウンロードして読み込んでください。",
  "settingsTitle": "旧サイトのデータを読み込む",
  "instructions": "以前使っていたブラウザーで秘伝書のバックアップをダウンロードし、ここで読み込んでください。",
  "link": "「グリーナーの依頼」でバックアップをダウンロード",
  "choose": "バックアップファイルを選択",
  "summary": "秘伝書 {library} 件 · 実験 {experiments} 件 · 開拓研究 {studies} 件 · 装備 {gear} 件 · お気に入り {favorites} 件 · 設定 {settings} 件",
  ...
  "later": "あとで",
  "errors": {
    "invalid": "不完全または無効なデータ：{detail}。何も読み込まれていません。バックアップを確認してください。",
    "format": "「グリーナーの依頼」の秘伝書 v1 バックアップを選択してください。工房や単独の秘伝書・実験ファイルには対応していません。",
    "tooLarge": "ファイルが 10 MB を超えています。",
    "storage": "ブラウザーのストレージに書き込めませんでした。元のデータは復元済みです。権限と空き容量を確認して再試行してください。",
    "rollback": "読み込みに失敗し、一部の元データを復元できませんでした。バックアップを保持し、編集を中止してストレージを確認してください。",
    "changed": "プレビュー後にデータが変更されました。ファイルを再選択して競合を確認してください。",
    "preference": "設定を保存できませんでした。ストレージの権限を確認してください。"
  }
},
  app: {
    title: '冷凍兔肉の秘伝書',
  ...
  createGuide: {
    ...
    searchPlaceholder: '日本語名を入力してください。見つからない場合は英語名も試せます',
  ...
  solver: {
    ...
    strategy: {
      ...
      workerErrors: {
        ...
        memoCapacity: {
          title: 'メモリ不足のため計算を完了できません',
          desc: 'この条件では、採集判断により大きな計算メモリが必要ですが、この端末で使える上限に達しました。条件を絞るか、より単純な状況でお試しください。',
          raiseBudget: 'メモリを増やして再試行',
          manualRisk: 'メモリを増やすと、ブラウザが一時的に遅くなったり、このタブが応答しなくなる場合があります。端末の空きリソースが少ない場合、システムがタブを閉じることがあります。'
        },
        memoAllocationFailed: {
          title: '高メモリ計算を開始できません',
          desc: 'ブラウザがこの採集計算に必要なメモリを確保できませんでした。条件を絞るか、より単純な状況でお試しください。'
        }
      },
      gatherAction: '採集',
  ...
  collectableSolver: {
    ...
    errors: {
      ...
      memoCapacity: { title: 'メモリ不足のため計算を完了できません', desc: 'この条件では、この端末で収集品判断に使えるメモリ上限に達しました。条件を絞るか、より単純な状況でお試しください。', raiseBudget: 'メモリを増やして再試行', manualRisk: 'メモリを増やすと、ブラウザが一時的に遅くなったり、このタブが応答しなくなる場合があります。端末の空きリソースが少ない場合、システムがタブを閉じることがあります。' },
      memoAllocationFailed: { title: '高メモリ計算を開始できません', desc: 'ブラウザがこの計算に必要なメモリを確保できませんでした。条件を絞るか、より単純な状況でお試しください。' },
  ...
  collectableStrategyLab: {
    ...
    analysis: {
      ...
      run: '分析する',
      running: '分析中',
      runningDesc: '方針の結果を計算しています。しばらくお待ちください。',
    ...
    editor: {
      ...
      calculatingNodes: 'ノードを計算中…',
      noManagedNodes: 'この方針はまだどのノードにも適用されていません。',
      effectPreview: '数値変化',
      effectPreviewDescription: '展開された方針ノードをもとに、この方針が適用された時の各アクション 1 回分の変化範囲を表示します。',
      ...
      effectMetrics: {
        ...
        scrutinyBonus: '純化値ボーナス {value}',
        valueIncreaseRate: '価値上昇率 {from} → {to}（{delta}、×1.75）',
        meticulousSaveRate: '慎重純化の耐久消費なし率 {from} → {to}（{delta}）',
        collectSuccessBonus: '採集成功率 {value}',
        nextCollectSuccessBonus: '次回採集成功率 {value}',
