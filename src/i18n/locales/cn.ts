export default {
  ...
  "instructions": "用原来的浏览器下载秘笈备份，再回来导入。",
  "link": "前往「拾穗人的委托」下载备份",
  "choose": "选择备份文件",
  "summary": "藏书库 {library} 条 · 实验 {experiments} 条 · 开拓研究 {studies} 条 · 装备 {gear} 组 · 收藏 {favorites} 条 · 设置 {settings} 项",
  ...
  "errors": {
    ...
    "format": "请选择「拾穗人的委托」导出的秘笈 v1 备份，不支持工坊或单条秘笈／实验文件。",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: '我遇到虫虫了！',
        a: '倘若在网站中遇到 Bug 或是使用上的疑问，欢迎前往 <a href="https://github.com/emu-rabbit/frozen_rabbit_tome/issues" target="_blank" rel="noreferrer" class="text-soft-green-600 hover:text-soft-green-700 font-bold underline decoration-dotted underline-offset-4 transition-colors">GitHub Issues</a> 告诉兔肉唷！'
      }
    ],
  ...
  createGuide: {
    ...
    searchPlaceholder: '请输入简体中文名称，若搜索不到可以试试英文名称',
    loading: '数据加载中，请稍候…',
  ...
  simulator: {
    ...
    emptyPrimaryRotation: '未加入技能时会以“不施放技能”分析；也可以点下方技能建立手法。',
  ...
  experimentDatabase: {
    ...
    snapshot: {
      staleBadge: '快照可能过期',
      staleKicker: '保存时快照',
      staleTitle: '这份分析快照可能已不是当前模型的结果',
      staleDesc: '载入后会保留你的输入与手法，但卡片上的摘要是保存当下的快照。建议回到实验台后重新分析一次。',
      loadAnyway: '载入实验',
      cancel: '取消'
    },
    actions: {
  ...
  tomeLibrary: {
    ...
    nodeState: {
      regular: '次数 +{gathering} / 获得 +{yield} / 额外率 +{extra}',
      collectable: '次数 +{gathering}'
    },
    relicBonus: {
      enabled: '已纳入',
      disabled: '未纳入'
    ...
    snapshot: {
      ...
      staleBadge: '快照可能过期',
      staleKicker: '保存时快照',
      staleTitle: '这张秘籍快照可能已不是当前模型的结果',
      staleDesc: '载入后会保留你的输入条件，但卡片上的推荐摘要是保存当下的快照。建议回到秘籍后重新求解一次。',
      loadAnyway: '载入秘籍'
    },
    actions: {
      edit: '打开副本',
